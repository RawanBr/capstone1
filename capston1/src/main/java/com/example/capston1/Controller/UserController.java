...
//------------------EXTRA----------------------------------------------------------------
    ...
    public ResponseEntity<?> addBalance (@PathVariable String userId, @PathVariable int amount) {
        if (userService.addBalance(userId, amount)) {
    ...
    @GetMapping("/get-affordable/{userId}")
    public ResponseEntity<?> getAffordableProducts (@PathVariable String userId) {
        ArrayList<Product> affordable = userService.getAffordableProducts(userId);
        ...
        if (affordable.isEmpty()) {
            ...
            if (!userExists) {
                ...
                return ResponseEntity.status(400).body(new ApiResponse("No affordable product found"));
            }
        }
        return ResponseEntity.status(200).body(userService.getAffordableProducts(userId));
    }
    ...
    public ResponseEntity<?> remainingBalance (@PathVariable String userId, @PathVariable String productId) {
        if (userService.remainingBalance(userId, productId) == 1) {
            return ResponseEntity.status(400).body(new ApiResponse("User not found"));
        }
        else if (userService.remainingBalance(userId, productId) == 2) {
            return ResponseEntity.status(400).body(new ApiResponse("Product not found"));
        }
        else if (userService.remainingBalance(userId, productId) == 3) {
            return ResponseEntity.status(400).body(new ApiResponse("User doesn't have enough balance"));
        }
        else {
            return ResponseEntity.status(200).body(userService.remainingBalance(userId, productId));
        }
    }
