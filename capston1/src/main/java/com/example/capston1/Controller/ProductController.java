...
//-------------------------------Extra------------------------------------------
    ...
    @GetMapping("/search/{categoryId}/{min}/{max}")
    public ResponseEntity<?> searchByCategory (@PathVariable String categoryId, @PathVariable int min, @PathVariable int max) {
        ArrayList<Product> results = productService.searchByCategory(categoryId, min, max);


        if (results == null) {
            return ResponseEntity.status(400).body(new ApiResponse("Category not found found"));
        }
